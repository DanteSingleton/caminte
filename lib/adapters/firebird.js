...
FB.prototype.key = function (name) {
    return this.schema.definitions[name].settings.primaryKeys[0] || 'id';
}

FB.prototype.create = function (name, data, callback) {
    var table = this.schema.tableName(name);
    var key =  this.schema.definitions[name].settings.primaryKeys[0] || 'id';
...
FB.prototype.destroy = function destroy(name, id, callback) {
    ...
    if (id) {
        var table = this.schema.tableName(name);
        var sql = 'DELETE FROM ' + quote(table) + ' WHERE "'+key+'" = ?';
...
FB.prototype.findById = function findById(name, id, callback) {    
    var key =  this.schema.definitions[name].settings.primaryKeys[0] || 'id';
    var table = this.schema.tableName(name);
    var sql = 'SELECT FIRST 1 * FROM ' + quote(table) + ' WHERE "'+key+'" = ?';
...
FB.prototype.exists = function count(name, id, callback) {    
    var key =  this.schema.definitions[name].settings.primaryKeys[0] || 'id';
    var table = this.schema.tableName(name);
    var sql = 'SELECT FIRST 1 "'+key+'" FROM ' + quote(table) + ' WHERE "'+key+'" = ?';
