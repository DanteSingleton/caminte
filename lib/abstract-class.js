        ...
        obj.trigger('create', function (done) {
            ...
            this._adapter().create(modelName, this.constructor._forDB(data), function (err, id, rev) {
                if (id) {
                    obj.__data.id = id;
                    obj.__dataWas.id = id;
                    defineReadonlyProp(obj, adapter.key(modelName), id);
                }
                if (rev) {
                    obj._rev = rev;
                }
...
AbstractClass.upsert = AbstractClass.updateOrCreate = function upsert(query, data, callback) {
    ...
    var p = deferPromise();
    var Model = this;
    var key =  this.schema.definitions[Model.modelName].settings.primaryKeys[0] || 'id';
    if (query[key]) {
        this.findById(query[key], function (err, inst) {
            if (err) {
                if(!callback) p.reject(err);
                return callback && callback(err);
            }
            if (inst) {
                inst.updateAttributes(data, function (err, internal_data) {
                    if (err) {
                        if(!callback) p.reject(err);
                    } else {
                        if(!callback) p.resolve();
                    }
                    return callback && callback(err, internal_data);
                });
            } else {
...
AbstractClass.prototype.updateAttributes = function updateAttributes(data, callback) {
    ...
    let key = this.constructor.schema.definitions[model].settings.primaryKeys[0] || 'id';
    if (!data) {
        data = {};
    }
    ...
    function update() {
        inst.trigger('save', function (saveDone) {
            inst.trigger('update', function (done) {
                ...
                inst._adapter().updateAttributes(model, inst[key], newData, function (err) {
                    if (!err) {
                        // update _was attrs
                        Object.keys(data).forEach(function (key) {
